...
class SegmentationNetwork(pl.LightningModule):
  ...
  def __init__(self, network: nn.Module, 
    ...
    # evaluation metrics for soil vs. vegetation
    self.metric_test_vegetation_iou = torchmetrics.JaccardIndex(num_classes=2, reduction=None)
  ...
  def training_step(self, batch: dict, batch_idx: int) -> Dict[str, Any]:
    ...
    if ('regular' in self.train_step_settings) and ('regular_ibn_net' in self.train_step_settings):
      raise ValueError('Invalid definition of training steps.')
    ...
    if 'regular_global_style_transfer' in self.train_step_settings:
      logits_regular_global_style_crops = self.forward(batch['img_global_style_crops'])
      logits_regular_global_style_weeds = self.forward(batch['img_global_style_weeds'])

      loss_regular_global_style_crops = self.compute_xentropy_loss(logits_regular_global_style_crops, batch['anno_global_style_crops'], mode='train')
      loss_regular_global_style_weeds = self.compute_xentropy_loss(logits_regular_global_style_weeds, batch['anno_global_style_weeds'], mode='train')
      processed_steps.append('regular_global_style_transfer')

    if 'sparse' in self.train_step_settings:
      logits_sparse_crops = self.forward(batch['img_crop_with_sparse_anno'])
      logits_sparse_weeds = self.forward(batch['img_weed_with_sparse_anno'])

      mask_keep_sparse_crops = batch['sparse_crop_anno'] != 255
      mask_keep_sparse_weeds = batch['sparse_weed_anno'] != 255

      loss_sparse_crop_img = self.compute_xentropy_loss(logits_sparse_crops, batch['sparse_crop_anno'], mode='train', mask_keep=mask_keep_sparse_crops)
      loss_sparse_weed_img = self.compute_xentropy_loss(logits_sparse_weeds, batch['sparse_weed_anno'], mode='train', mask_keep=mask_keep_sparse_weeds)
      processed_steps.append('sparse')
  ...
  def validation_step(self, batch: dict, batch_idx: int) -> Dict[str, Any]:
    ...
    elif 'regular_ibn_net' in self.val_step_settings:
      logits = self.forward_ibn_net(batch['input_image'])
    else:
      raise ValueError
  ...
  def test_step(self, batch: dict, batch_idx: int) -> Dict[str, Any]:
    ...
    if 'regular' in self.test_step_settings:
      logits = self.forward(batch['input_image'])
    elif 'regular_ibn_net' in self.test_step_settings:
      logits = self.forward_ibn_net(batch['input_image'])
    else:
      raise ValueError
    ...
    # evaluation of soil vs. vegetatation
    pred_vegetation = pred_cls.clone()
    pred_vegetation[pred_vegetation > 0] = 1 # set all vegetation to 1
    
    ground_truth_vegetation = batch['anno'].clone()
    ground_truth_vegetation[ground_truth_vegetation > 0] = 1 # set all vegetation to 1

    self.metric_test_vegetation_iou(pred_vegetation, ground_truth_vegetation)
  ...
  def test_epoch_end(self, test_step_outputs: List) -> None:
    ...
    iou_soil_vegetation = self.metric_test_vegetation_iou.compute()
    self.metric_test_vegetation_iou.reset()
    ...
    path_to_soil_vegetation_iou_dir = os.path.join(self.trainer.log_dir, 'evaluation', 'iou-soil-vegetation',  f'epoch-{epoch:06d}')
    save_iou_metric(iou_soil_vegetation, path_to_soil_vegetation_iou_dir)

  def predict_step(self, batch, batch_idx):
    if not self.predict_step_settings:
      raise ValueError('You need to specify the settings for the predict step.')
    assert len(self.predict_step_settings) == 1

    # predictions
    if 'regular' in self.predict_step_settings:
      logits = self.forward(batch['input_image'])
    elif 'regular_ibn_net' in self.predict_step_settings:
       logits = self.forward_ibn_net(batch['input_image'])
    else:
      raise ValueError
