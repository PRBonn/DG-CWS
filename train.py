...
def main():
  ...
  if cfg.get('train').get('ckpt_every_n_epochs') is not None:
    checkpoint_saver_steps = ModelCheckpoint(
        filename=cfg['experiment']['id'] + '_{epoch:02d}_{train_mIoU:.4f}_{val_mIoU:.4f}', every_n_epochs=cfg['train']['ckpt_every_n_epochs'], save_top_k=-1)
